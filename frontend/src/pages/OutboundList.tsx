...
const ORDER_TYPE_LABELS: Record<OutboundOrderType, string> = {
  sales: '销售',
  manual: '手动',
  scrap: '报废',
};

const STATUS_TAGS: Record<OutboundOrderStatus, { color: string; label: string }> = {
  draft: { color: 'blue', label: '草稿' },
  completed: { color: 'green', label: '已完成' },
  cancelled: { color: 'default', label: '已取消' },
};

const STATUS_OPTIONS: { value: OutboundOrderStatus | ''; label: string }[] = [
  { value: '', label: '全部' },
...
export default function OutboundList() {
  ...
  const queryParams: OutboundOrderListQuery = {
    keyword: keyword || undefined,
    warehouse_id: warehouseId || undefined,
    status: (status || undefined) as OutboundOrderStatus | undefined,
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['outboundOrders', queryParams],
    queryFn: () => listOutboundOrders(queryParams),
  });
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'order_type',
      render: (t: OutboundOrderType) => ORDER_TYPE_LABELS[t] ?? t,
    },
    {
      title: '状态',
      dataIndex: 'status',
      key: 'status',
      render: (s: OutboundOrderStatus) => {
    ...
    },
    {
      title: '操作',
      key: 'actions',
      render: (_: unknown, r: OutboundOrderListItem) => (
        <Space>
          <Button
            type="link"
            size="small"
    ...
    <div>
      <Space style={{ marginBottom: 16 }} wrap>
        ...
        <Button
          type="primary"
          icon={<PlusOutlined />}
          onClick={() => navigate('/outbounds/new')}
        >
          新增出库单
        </Button>
      </Space>
