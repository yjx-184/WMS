...
export default function OutboundDetail() {
  ...
  const completeMutation = useMutation({
    mutationFn: (req: { items: { item_id: string; actual_qty: number }[] }) =>
      completeOutboundOrder(id!, req),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['outboundOrders'] });
      queryClient.invalidateQueries({ queryKey: ['outboundOrder', id] });
  ...
  const cancelMutation = useMutation({
    mutationFn: () => cancelOutboundOrder(id!),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['outboundOrders'] });
      queryClient.invalidateQueries({ queryKey: ['outboundOrder', id] });
      message.success('已取消');
