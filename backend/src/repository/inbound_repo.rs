use crate::dto::inbound::{
    InboundOrderDetailItem, InboundOrderDetailResponse, InboundOrderListItem,
};
use crate::model::inbound::{InboundOrder, InboundOrderItem, InboundOrderStatus, InboundOrderType};
...
impl InboundOrderRepository {
    ...
    pub async fn list(
        ...
        page: u32,
        page_size: u32,
    ) -> Result<(Vec<InboundOrderListItem>, i64), sqlx::Error> {
        ...
        let total: (i64,) = sqlx::query_as(
            ...
            WHERE ($1::text IS NULL OR o.order_no ILIKE $1)
              ...
              AND ($3::inbound_order_status IS NULL OR o.status = $3)
        ...
        let items: Vec<InboundOrderListItem> = sqlx::query_as(
            r#"
    ...
    pub async fn find_by_id_with_items(
        pool: &PgPool,
        id: Uuid,
    ) -> Result<Option<InboundOrderDetailResponse>, sqlx::Error> {
        #[derive(sqlx::FromRow)]
        struct DetailHeader {
            id: Uuid,
            order_no: String,
        ...
        match header {
            None => Ok(None),
            Some(h) => {
                let items: Vec<InboundOrderDetailItem> = sqlx::query_as(
                    r#"
                ...
                Ok(Some(InboundOrderDetailResponse {
                    id: h.id,
                    order_no: h.order_no,
    ...
    pub async fn find_by_id_exec<'e, E: Executor<'e, Database = Postgres>>(
        ...
        sqlx::query_as(
            ...
            FROM inbound_orders WHERE id = $1
            "#,
        )
        .bind(id)
    ...
    pub async fn insert_order<'e, E: Executor<'e, Database = Postgres>>(
        ...
        order_type: InboundOrderType,
        remark: Option<&str>,
    ) -> Result<InboundOrder, sqlx::Error> {
        sqlx::query_as(
            r#"
            INSERT INTO inbound_orders (order_no, warehouse_id, order_type, remark)
    ...
    ///
    /// Takes `&mut E` (not `E` by value) so the caller can pass
    /// `&mut *tx` (where `tx` is a `Transaction`) and have every row
        ...
        for<'a> &'a mut E: Executor<'a, Database = Postgres>,
    {
        ...
        for (product_id, location_id, planned_qty) in items {
            let item = sqlx::query_as(
                r#"
                INSERT INTO inbound_order_items (order_id, product_id, location_id, planned_qty)
                VALUES ($1, $2, $3, $4) RETURNING *
                "#,
            )
            .bind(order_id)
    ...
    pub async fn delete_items<'e, E: Executor<'e, Database = Postgres>>(
        ...
        sqlx::query("DELETE FROM inbound_order_items WHERE order_id = $1")
            .bind(order_id)
            .execute(executor)
            .await?;
        Ok(())
    ...
    pub async fn update_item_actual<'e, E: Executor<'e, Database = Postgres>>(
        ...
        sqlx::query("UPDATE inbound_order_items SET actual_qty = $2 WHERE id = $1")
            .bind(item_id)
            .bind(actual_qty)
            .execute(executor)
            .await?;
